...
const Stock = () => {
  ...
  const fetchStock = () => {
    axios
      .get(`http://localhost:80/api/stock`)
      .then(response => {
  ...
  const updateStock = (productId, availableQuantity) => {
    axios
      .post(`http://localhost:80/api/stock/updateStock?productId=${productId}&availableQuantity=${availableQuantity}`)
      .then(response => {
        fetchStock()
      })
  }
  ...
  const setStock = () => {
    axios
      .post(`http://localhost:80/api/stock/setStock?productId=${formProductId}&availableQuantity=${formAvailableQuantity}`)
      .then(response => {
        fetchStock()
      })
  }
  ...
  const removeAllStock = () => {
    axios
      .delete(`http://localhost:80/api/stock/deleteAll`)
      .then(response => {
        setStockInfo([])
      })
  }
